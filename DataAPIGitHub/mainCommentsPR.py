...
def get_data(owner, name, number):
    url = 'https://api.github.com/repos/{}/{}/pulls/{}/comments'.format(owner, name, str(number))
    ...
    for dataComment in data:
        ...
        comment['pull_request']=dataComment['_links']['pull_request']['href']
        comment['pull_request_review_id'] = dataComment['pull_request_review_id']
        ...
        db.pullsComments.insert_one(comment)
...
def collect_pulls(owner, name):
    ...
    while not last_page:
        ...
        for data in data_all:
            get_data(owner, name, data['number'])
